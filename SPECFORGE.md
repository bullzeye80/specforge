...
| `skills/plg-flow/` | **v1 authored + dogfooded (ownership-gated).** v0 breadboard rung (`flow-graph.json` §7.2 → `straight-line.md` §7.3) shipped + dogfooded on anchor-shaping. **v1 adds the fat-marker "what" rung:** one `wireframe.json` (§8.1, `version: "2"`) per screen **this feature owns** (`shape.md` `owns-screens` ∩ screens the flow touches — §7.1), never a screen it only references; regions + contents, structural only (no layout/grid/states/copy), each interactive element traced to a flow-graph affordance, v0.2 `action:<object>.<verb>` refs. **Screen-ownership decision (see §7.1):** each screen has exactly one owning feature declared in `shape.md` `owns-screens`; plg-flow wireframes only owned screens. The anchor-shaping dogfood originally authored 2 screens; under the rule it keeps only `screen:shape/anchor-result` (the surface it owns) — `screen:onboarding/describe-idea` belongs to **idea-intake** and is authored by that feature's own pass. Next rung — `low-fi.json` (layout half). |
