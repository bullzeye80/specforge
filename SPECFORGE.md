...
| `skills/plg-flow/` | **v1 authored** — v0 breadboard rung (`flow-graph.json` §7.2 → `straight-line.md` §7.3) shipped + dogfooded on anchor-shaping (First-Strike anchored on `event:shaping.artifact-generated`). **v1 adds the fat-marker "what" rung:** one `wireframe.json` (§8.1, `version: "2"`) per distinct screen the flow touches — regions + contents, structural only (no layout/grid/states/copy), each interactive element traced back to a flow-graph affordance and using v0.2 `action:<object>.<verb>` refs. **Dogfooded on anchor-shaping** — authored 2 of the 4 screens the flow touches (`describe-idea` entry + `anchor-result` first-strike) and correctly omitted the 2 terminal exit destinations (`product/tree`, `dashboard/main`) that have empty affordance arrays, rather than fabricate their regions (anti-fabrication + traceability rules). Both wireframes cross-validate (refs resolve, structural-only, `action:shaping.artifact.generate` = domain-map command matching its affordance). Next rung — `low-fi.json` (layout half of the fat-marker). |
